...
int inOrderF(ww node, int *table, int it, int *res)
{
    if (node)
    {
        it = inOrderF(node->lewy, table, it, res);
        if (*res == 0)
            return it;
        *res = 0;
        if (table[it] == node->wartosc)
            *res = 1;
        it++;
        if (*res == 0)
            return it;
        it = inOrderF(node->prawy, table, it, res);
    }
    return it;
}
...
int diffTree(ww t1, ww t2, int ctree)
{
    int *table;
    int res = 1;
    table = calloc(ctree, sizeof(int));
    AddToArray(t1, table, 0);
    inOrderF(t2, table, 0, &res);
    free(table);
    return res;
}
