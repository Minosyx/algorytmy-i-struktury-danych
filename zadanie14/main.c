    ...
    while(bool){
        ...
        switch (choice){
            ...
            case 3:
                ...
                clear_screen();
                element = find(&tree, x);
                if (*element) printf("Element o wartosci %s lezy pod adresem %p\n\n", (*element)->wartosc, (*element));
                else printf("Brak elementu o podanej wartosci w drzewie!\n\n");
                break;
            case 4:
                element = min(&tree);
                if(*element) printf("Wartosc najmniejsza to %s\n\n", (*element)->wartosc);
                else printf("Drzewo jest puste!\n\n");
                break;
            case 5:
                element = max(&tree);
                if(*element) printf("Wartosc najwieksza to %s\n\n", (*element)->wartosc);
                else printf("Drzewo jest puste!\n\n");
                break;
            case 6:
                ...
                if (*element) {
                    tmp = prev(*element);
                    if (tmp) printf("Poprzednikiem jest element o wartosci %s\n\n", tmp->wartosc);
                    else printf("Brak poprzednika dla podanej wartosci\n\n");
                }
                else printf("Brak elementu o podanej wartosci w drzewie!\n\n");
                break;
            case 7:
